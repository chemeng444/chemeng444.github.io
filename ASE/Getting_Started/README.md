...
[`run_111.py`](run_111.py) is a script that sets up the Quantum ESPRESSO calculator and performs the geometry optimization with respect to energy. This must be submitted to an external queue and should not be run directly in the login node. Make sure that you have run `setup_111.py` to generate your `slab.traj` file, then submit the optimization script using:

```bash
$ sbatch --job-id=$PWD run_111.py
```
...
**Requirement:** Plot the change in the total slab energy as a function of the different k-points. By default the 'gamma' keyword can be used when only 1 k-point is needed in all directions. Otherwise, specify all three k-points in the script.
